...
public class FolderPlaySong extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        loopButton.setOnClickListener(v -> {
            isLooping = !isLooping;
            loopButton.setColorFilter(isLooping ? 0xFF00FF00 : 0xFFFFFFFF); // Change color to indicate loop status
        });

        shuffleButton.setOnClickListener(v -> {
            isShuffling = !isShuffling;
            shuffleButton.setColorFilter(isShuffling ? 0xFF00FF00 : 0xFFFFFFFF); // Change color to indicate shuffle status
            if (isShuffling) {
    ...
    private void playSong(int position) {
        ...
        mediaPlayer.setOnCompletionListener(mp -> {
            if (isLooping) {
                playSong(position); // Loop the current song
            } else {
                playNextSong(); // Call the method to play the next song
            }
        });
